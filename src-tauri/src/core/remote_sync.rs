...
pub fn discover_agent_directories(host: &str) -> Result<Vec<RemoteAgentDirectory>> {
    ...
    for adapter in default_tool_adapters() {
        command.push_str(&format!(
            ...
            detect = shell_quote(&adapter.relative_detect_dir),
            skills = shell_quote(&adapter.relative_skills_dir),
            key = shell_quote(&adapter.key),
            name = shell_quote(&adapter.display_name),
            path = shell_quote(&format!("~/{}", adapter.relative_skills_dir)),
        ));
    }
